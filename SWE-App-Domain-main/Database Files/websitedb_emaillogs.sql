...
CREATE TABLE `emaillogs` (
  ...
  `email_type` enum('approval','expiration','notification') DEFAULT NULL,
  `sent_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `status` enum('sent','failed','pending') DEFAULT 'pending',
  PRIMARY KEY (`email_id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `emaillogs_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`)
...
-- Dump completed on 2025-09-29 16:23:56
